...
const REVIEW_LINK = process.env.REVIEW_LINK || 'https://t.me/yanvismokcoded/12'; // ссылка, куда просим оставить отзыв после выполнения заказа
...
/** Отправляет покупателю сообщение с просьбой оставить отзыв — вызывается
 *  один раз в момент, когда заказ реально считается выполненным (успешная
 *  автовыдача через twiboost или ручная отметка "Выдано" в /admin). */
function sendReviewRequest(userId) {
  bot.sendMessage(
    userId,
    `✅ Ваш заказ выполнен!\n\nБудем очень благодарны, если оставите отзыв — это помогает нам развиваться:\n${REVIEW_LINK}`
  ).catch(err => console.error('Не удалось отправить просьбу об отзыве:', err.message));
}
...
app.post('/admin/api/orders/:userId/:orderId/status', (req, res) => {
  ...
  const resolvedUserId = users.has(Number(req.params.userId)) ? Number(req.params.userId) : req.params.userId;
  const user = users.get(resolvedUserId);
  const order = user && user.orders.find(o => o.id === req.params.orderId);
  if (!order) return res.status(404).json({ error: 'Заказ не найден' });

  // Просьбу оставить отзыв шлём только в момент ПЕРЕХОДА в статус
  // "выполнено" — если заказ уже был в таком статусе, повторно не пишем.
  const wasCompleted = /выполнен|выдано/i.test(order.status || '');
  const willBeCompleted = /выполнен|выдано/i.test(status);

  order.status = status;
  saveData();
  if (willBeCompleted && !wasCompleted) sendReviewRequest(resolvedUserId);
...
app.post('/admin/api/premium-reactions/mark-done', (req, res) => {
  ...
  const resolvedUserId = users.has(Number(userId)) ? Number(userId) : userId;
  const user = users.get(resolvedUserId);
  ...
  saveData();
  sendReviewRequest(resolvedUserId);
  res.json({ success: true });
});
