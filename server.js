...
// Провайдеры оплаты картой, подключённые в @BotFather → Bot Settings →
// Payments. Токены там же (для теста и для боевого режима — токены разные,
// не перепутайте). Оба провайдера принимают ₽ — конвертация из внутренних
// цен в Stars одна и та же для обоих (см. STARS_TO_RUB ниже).
const SMARTGLOCAL_PROVIDER_TOKEN = process.env.SMARTGLOCAL_PROVIDER_TOKEN;
const REDSYS_PROVIDER_TOKEN = process.env.REDSYS_PROVIDER_TOKEN;
// Курс конвертации ⭐ → ₽ для оплаты картой (весь магазин ценообразован в
// Stars, а провайдеры карт принимают рубли — поэтому при оплате картой сумма
...
// Используется для обоих карточных провайдеров.
const CARD_MIN_KOPECKS = Number(process.env.SMARTGLOCAL_MIN_KOPECKS || process.env.CARD_MIN_KOPECKS) || 10000;

// Способы оплаты картой: ключ — то же значение, что фронтенд шлёт в поле
// provider ('card_sg' / 'card_redsys'), значение — токен и подпись для
// уведомлений владельцу. Добавить ещё одного провайдера — значит добавить
// сюда одну строку (и токен в .env), остальной код трогать не придётся.
const CARD_PROVIDERS = {
  card_sg: { token: SMARTGLOCAL_PROVIDER_TOKEN, label: 'картой (Smart Glocal)', currency: 'RUB' },
  card_redsys: { token: REDSYS_PROVIDER_TOKEN, label: 'картой (Redsys)', currency: 'RUB' },
};
...
if (!SMARTGLOCAL_PROVIDER_TOKEN) {
  console.warn(
    'SMARTGLOCAL_PROVIDER_TOKEN не задан в .env — оплата картой через Smart Glocal будет недоступна.\n' +
    'Возьмите токен в @BotFather → Bot Settings → Payments → Smart Glocal.'
  );
}
if (!REDSYS_PROVIDER_TOKEN) {
  console.warn(
    'REDSYS_PROVIDER_TOKEN не задан в .env — оплата картой через Redsys будет недоступна.\n' +
    'Возьмите токен в @BotFather → Bot Settings → Payments → Redsys.'
  );
}
...
// Сколько копеек нужно выставить в инвойсе карточного провайдера, чтобы
// списать эквивалент stars ⭐ по курсу STARS_TO_RUB, с округлением вверх
// (чтобы магазин никогда не терял на округлении — как и с округлением
// Stars) и не ниже минимума провайдера.
function starsToRubKopecks(stars) {
  const kopecks = Math.ceil(stars * STARS_TO_RUB * 100 - 1e-9);
  return Math.max(CARD_MIN_KOPECKS, kopecks);
}
...
app.post('/api/create-invoice', async (req, res) => {
  try {
    ...
    // provider: 'stars' (по умолчанию) или один из ключей CARD_PROVIDERS
    // ('card_sg' — Smart Glocal, 'card_redsys' — Redsys).
    const payProvider = CARD_PROVIDERS[provider] ? provider : 'stars';
    ...
    // Если баланса хватило на всю сумму — платить Stars/картой не нужно
    // вообще, поэтому проверка на доступность карты (ниже) для этого случая
    // не имеет значения и намеренно НЕ блокирует такой заказ.
    if (total <= 0) {
    ...
    // С этого места остаток > 0 действительно нужно на что-то выставить —
    // вот тут и проверяем, что выбранный способ оплаты доступен.
    const cardCfg = CARD_PROVIDERS[payProvider];
    if (cardCfg && !cardCfg.token) {
      return res.status(400).json({ error: 'card_unavailable', message: 'Этот способ оплаты картой временно недоступен, выберите другой' });
    }

    const description = items.map(i => `${i.name} × ${i.qty}`).join(', ').slice(0, 250);

    if (cardCfg) {
      // Карточные провайдеры принимают только реальную валюту (₽), а не
      // Stars — конвертируем сумму заказа в копейки по курсу STARS_TO_RUB, с
      ...
      const roundingCredit = round2(chargedAmountToStars(chargedKopecks, cardCfg.currency) - total);

      pendingOrders.set(orderId, {
        type: 'order', userId: tgUser.id, items, subtotal, balanceUsed,
        finalTotal: total, currency: cardCfg.currency, providerLabel: cardCfg.label, chargedKopecks, roundingCredit,
      });
      ...
      const invoiceLink = await bot.createInvoiceLink(
        ...
        description,
        orderId,
        cardCfg.token,       // provider_token — обязателен для реальной валюты
        cardCfg.currency,    // валюта провайдера
...
app.post('/api/create-topup-invoice', async (req, res) => {
  try {
    ...
    if (cardCfg) {
      ...
      pendingOrders.set(orderId, { type: 'topup', userId: tgUser.id, amount: stars, currency: cardCfg.currency, providerLabel: cardCfg.label, chargedKopecks });
...
bot.on('message', async (msg) => {
  ...
  const methodLabel = order?.providerLabel || (currency === 'RUB' ? 'картой' : 'Stars');
