...
const BALANCE_PROMO_CODES = {
  ...
  PHYSICS: { value: 10, maxUses: 5, uses: 0, redeemedBy: new Set(), label: 'Промокод PHYSICS — +10 ⭐ на баланс' },
};
...
app.post('/api/redeem-promo', async (req, res) => {
  ...
  // Уведомляем владельца о каждом использовании промокода.
  // Ошибка отправки (например, бот не может написать OWNER_CHAT_ID)
  // не должна ломать сам ответ пользователю — промокод уже применён.
  const promoUserName = [tgUser.first_name, tgUser.last_name].filter(Boolean).join(' ') || tgUser.username || tgUser.id;
  bot.sendMessage(
    OWNER_CHAT_ID,
    `🎟 Промокод "${key}" использован\n` +
    `Пользователь: ${promoUserName}${tgUser.username ? ` (@${tgUser.username})` : ''} [ID: ${tgUser.id}]\n` +
    `Начислено: +${promo.value} ⭐\n` +
    `Использований: ${promo.uses}/${promo.maxUses}`
  ).catch(err => console.error('Не удалось отправить уведомление об использовании промокода:', err && err.message));
...
function orderToHistoryEntry(userId, items, total, status, refundAmount = 0) {
  return {
    ...
    total,
    status,
    // Сколько вернуть на баланс, если панель twiboost сообщит об ошибке/отмене
    // заказа (списанный баланс + реально оплаченные Stars). refunded — защита
    // от повторного начисления, если статус ошибки придёт больше одного раза.
    refundAmount,
    refunded: false,
  };
}
...
async function fulfillOrder(historyEntry, items) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      // отслеживает только twiboostOrderIds), поэтому возвращаем деньги
  ...
  if (failedAmount > 0) {
    ...
    if (buyer) {
      ...
      // Уменьшаем сумму, которую ещё можно будет вернуть по этому же заказу
      // позже через pollTwiboostStatuses (если оставшиеся, уже успешно
      // созданные заявки тоже провалятся в панели) — иначе одна и та же
      // позиция могла бы вернуть деньги дважды.
      historyEntry.refundAmount = round2(Math.max(0, (historyEntry.refundAmount || 0) - failedAmount));
  ...
  if (twiboostOrderIds.length === items.length) {
...
async function pollTwiboostStatuses() {
  ...
  for (const [historyEntryId, meta] of pollingOrders) {
    ...
    if (rows.every(r => String(r.status).toLowerCase() === 'completed')) {
      ...
      // Панель сообщила об ошибке/отмене — возвращаем деньги на баланс.
      // entry.refunded защищает от повторного начисления, если по этому
      // заказу ещё раз придёт статус ошибки.
      if (!entry.refunded && entry.refundAmount > 0) {
        user.balance = round2(user.balance + entry.refundAmount);
        entry.refunded = true;
        entry.status = `Ошибка выполнения — ${entry.refundAmount} ⭐ возвращены на баланс`;
        saveData();

        bot.sendMessage(
          meta.userId,
          `⚠️ По заказу возникла ошибка на стороне поставщика, поэтому мы вернули ${entry.refundAmount} ⭐ на ваш баланс. Приносим извинения за неудобства.`
        ).catch(err => console.error('Не удалось уведомить пользователя о возврате:', err.message));

        bot.sendMessage(
          OWNER_CHAT_ID,
          `↩️ Автоматический возврат ${entry.refundAmount} ⭐ на баланс пользователя (id ${meta.userId}) — заказ "${entry.items.map(i => i.name).join(', ')}" завершился ошибкой в панели.`
